      ...
      var pointSeries = chart.series.push(am5map.MapPointSeries.new(root, {}));
        ...
        pointSeries.bullets.push(function () {
          var circle = am5.Circle.new(root, {
            radius: 4,
            tooltipY: 0,
            fill: am5.color(0xffba00), //0xffba00
...
displays_activities_note()



  // displays activityes note
  ...
  const displays_active_page = ()=>{
    ...
    sections.forEach((section)=>{
      section.classList.contains(activePage)? section.classList.remove("hidden") : section.classList.add("hidden")
    })
    console.log("start");
  ...
  const navs = document.querySelectorAll(".navbar div")
  navs.forEach((nav)=>{
      nav.addEventListener("click", ()=>{
        nav.classList.add("active")
        activePage = nav.textContent?.trim()!
        displays_active_page()
      }) 
  })
